...
#if defined(KISAK_IOS) && !__has_include(<buildnumber.h>)
// buildnumber.h is generated by scripts/increment_build.* as a pre-build step on
// win32; the iOS census/lib build (scripts/platform/ios/build-engine-lib.sh) has no
// pre-build stage yet, so mirror the generated header with build 0.
// KISAKTODO(build): run increment_build.sh from the iOS build and drop this fallback.
...
#ifdef KISAK_IOS
// q_shared.h only defines CPUSTRING inside its WIN32 block ("win-x86"/...);
// give the iOS build its own version-string tag.
#ifndef CPUSTRING
...
void Com_Prefetch(const void* s, const uint32_t bytes, e_prefetch type)
{
	...
	case PRE_READ_WRITE:
        ...
        // DI: lol
#ifdef KISAK_IOS
        // winnt.h PreFetchCacheLine(PF_NON_TEMPORAL_LEVEL_ALL, ...) → clang
        // builtin: read prefetch, locality 0 (non-temporal, matches NTA hint)
...
void Com_OpenLogFile()
{
...
#ifdef KISAK_IOS
    time_t aclock; // MSVC __time64_t → time_t (already 64-bit on arm64-apple)
#else
    ...
    if (Sys_IsMainThread() && !opening_qconsole)
    {
        opening_qconsole = 1;
#ifdef KISAK_IOS
        // MSVC _time64/_localtime64 → libc equivalents (64-bit time_t natively)
        time(&aclock);
