...
{
    // no console input line exists on iOS; the in-game console (~) still works
    return NULL;
}
...
// ------------------------------------------------------------------
// System event queue — ring-buffer logic ported verbatim from win_main.cpp;
// only the Win32 message pump is gone (UIKit input arrives via Sys_QueEvent).
// ------------------------------------------------------------------
...
sysEvent_t *__cdecl Win_GetEvent(sysEvent_t *result)
{
    ...
    {
        // win32 pumps PeekMessageA/DispatchMessageA into MainWndProc here; on
        // iOS the main runloop already ran and queued any input events.
